      ...
      Object.keys(installedAndRegisteredModulesMap).forEach((key) => {
        ...
        if (hasMatchingNode) {
          dynamicMetadata[moduleTypeKey] = {
            type: "string",
            options: installedAndRegisteredModulesMap[moduleTypeKey].map(
              (module) => ({
                value: module.name,
                label: module.label,
              })
  ...
  const initialData = React.useMemo(() => {
    ...
    if (selectedGraph) {
      Object.keys(installedAndRegisteredModulesMap).forEach((key) => {
        ...
        if (hasMatchingNode) {
          dynamicInitialData[moduleTypeKey] =
            getGraphNodeAddonByName(moduleTypeKey)?.addon;
        }
      });
    }
          ...
          <GraphModuleCfgForm
            ...
            onUpdate={async (data, tools) => {
              try {
                ...
                const agoraRtcNode = GraphEditor.findNode(
                  selectedGraphCopy,
                  "agora_rtc"
                );
                if (!agoraRtcNode) {
                ...
                Object.entries(data).forEach(([key, value]) => {
                  ...
                  if (node && value && node.addon !== value) {
                    node.addon = value;
                    node.property = addonModules.find(
                ...
                const reasoningNodesWithVisualSupport =
                  GraphEditor.findNodeByPredicate(selectedGraphCopy, (node) => {
                    ...
                    if (module.type === ModuleRegistry.ModuleType.LLM) {
                      ...
                      return (
                        isLLM(node.name) &&
                        llmModule.options.inputModalities.includes(
                    ...
                    if (module.type === ModuleRegistry.ModuleType.V2V) {
                      ...
                      return (
                        isLLM(node.name) &&
                        v2vModule.options.inputModalities.includes(
                          ModuleRegistry.Modalities.Video
                        )
                      );
                    }
                ...
                const removedTools = currentToolsInGraph.filter(
                ...
                if (tools.length > 0) {
                  if (!enableRTCVideoSubscribe) {
                    enableRTCVideoSubscribe = tools.some((tool) =>
                      tool.includes("vision")
                    );
                  }
                  tools.forEach((tool) => {
                    if (!currentToolsInGraph.includes(tool)) {
                      const toolModule = addonModules.find(
                        (module) => module.name === tool
                      );
                      ...
                      const toolNode = GraphEditor.addNode(
                        selectedGraphCopy,
                        tool,
                        tool,
                        "default",
                        toolModule.defaultProperty
                      );

                      // Create or update connections
                      const llmNode = GraphEditor.findNodeByPredicate(
                        selectedGraphCopy,
                        (node) => isLLM(node.name)
                      );
                      if (llmNode) {
                        GraphEditor.linkTool(
                          selectedGraphCopy,
                          llmNode,
                          toolNode,
                          toolModuleRegistry[tool]
                        );
                      }
                    }
                  });
                ...
                GraphEditor.enableRTCVideoSubscribe(
                  selectedGraphCopy,
                  enableRTCVideoSubscribe
                );
...
const GraphModuleCfgForm = ({
  ...
  const toolModules = React.useMemo(() => {
    ...
    if (activeModule) {
      ...
      return allToolModules.filter((module) =>
        compatibleToolNames.includes(module.name)
      );
    }
                                    ...
                                    >
                                      {toolModules.length > 0 ? (
                                        toolModules.map((module) => (
                                          <DropdownMenuItem
                                            key={module.name}
                                            disabled={selectedTools.includes(
                                              module.name
                                            )} // Disable if the tool is already selected
                                            onClick={() => {
                                              if (
