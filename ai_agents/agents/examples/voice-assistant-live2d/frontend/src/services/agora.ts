...
export class AgoraService {
  ...
  private transcriptListeners: Set<(message: TranscriptMessage) => void> =
    new Set();
  ...
  private handleStreamMessage(streamData: any) {
    try {
      ...
      const [messageId, partIndexStr, totalPartsStr, content] =
        ascii.split("|");
  ...
  addTranscriptListener(
    listener: (message: TranscriptMessage) => void
  ): () => void {
