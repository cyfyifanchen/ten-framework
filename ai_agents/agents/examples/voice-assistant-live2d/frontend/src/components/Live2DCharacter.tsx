...
const Live2DCharacter = forwardRef<Live2DHandle, Live2DCharacterProps>(
  function Live2DCharacter(
    ...
    useEffect(() => {
      ...
      defaultExpressionRef.current = (expressions ?? []).find(
    ...
    useEffect(() => {
      ...
      idleMotionNameRef.current = (motions ?? []).find(
        (motion) => motion.autoPlay
      )?.name;
    }, [motions]);
    ...
    const applyDefaultExpression = useCallback(async (modelInstance: any) => {
      ...
      if (!defaultExpression) {
        const manager =
          modelInstance.internalModel?.motionManager?.expressionManager;
      ...
      try {
        ...
        console.warn(
          "[Live2DCharacter] Unable to apply default expression",
          defaultExpression,
          error
        );
        return false;
    ...
    const ensureIdleMotion = useCallback(
      async (modelInstance: any) => {
        ...
        try {
          ...
          console.warn(
            "[Live2DCharacter] Unable to start idle motion",
            idleName,
            error
          );
          return false;
    ...
    const setExpression = useCallback(
      async (name?: string) => {
        ...
        if (!name) {
          ...
          if (defaultName) {
            try {
              ...
              console.warn(
                "[Live2DCharacter] Unable to fallback to default expression",
                defaultName,
                error
              );
              return false;
        ...
        if (!expressionMapRef.current.has(name)) {
          console.warn(
            `[Live2DCharacter] Expression "${name}" is not registered for ${modelPath}`
          );
        }
        try {
          ...
          console.warn(
            "[Live2DCharacter] Unable to apply expression",
            name,
            error
          );
          return false;
    ...
    const setRandomExpression = useCallback(async () => {
      ...
      try {
        ...
        console.warn(
          "[Live2DCharacter] Unable to apply random expression",
          error
        );
        return false;
      }
    ...
    const playMotion = useCallback(
      async (name: string, options?: { priority?: number }) => {
        const model = live2dModelRef.current;
        if (!model) {
          return false;
        }
        ...
        if (!motionConfig) {
          console.warn(
            `[Live2DCharacter] Motion "${name}" is not registered for ${modelPath}`
          );
          return false;
        }
        try {
    ...
    const resolveMouthParameters = (coreModel: any, provided?: MouthConfig) => {
      ...
      try {
        ...
        if (Array.isArray(ids)) {
          ...
          ids &&
          typeof (ids as ArrayLike<string>).length === "number"
        ) {
    ...
    const ensureAudioContext = async () => {
      ...
      if (!audioContextRef.current) {
        ...
        if (!AudioCtx) {
          console.warn(
            "[Live2DCharacter] Web Audio API is not available; fallback lip sync disabled."
          );
          return null;
        }
      ...
      if (audioContextRef.current.state === "suspended") {
        try {
          ...
          console.warn(
            "[Live2DCharacter] Unable to resume AudioContext:",
            error
          );
        }
      }
    ...
    const startFallbackLipSync = async (stream: MediaStream) => {
      ...
      try {
        ...
        const animateMouth = () => {
          ...
          try {
            ...
            if (config.open) {
              ...
              if (config.up) {
                const positiveSpan = Math.max(
                  config.up.max - config.up.defaultValue,
                  0
                );
                const negativeSpan = Math.max(
                  config.up.defaultValue - config.up.min,
                  0
                );
                let targetUp = config.up.defaultValue;
                if (positiveSpan >= negativeSpan) {
                  targetUp =
                    config.up.defaultValue + smoothed * (positiveSpan || 1);
                } else {
                  targetUp =
                    config.up.defaultValue - smoothed * (negativeSpan || 1);
                }
                applyMouthParameter(config.up, targetUp);
              }
              if (config.down) {
                const positiveSpan = Math.max(
                  config.down.max - config.down.defaultValue,
                  0
                );
                const negativeSpan = Math.max(
                  config.down.defaultValue - config.down.min,
                  0
                );
                let targetDown = config.down.defaultValue;
                if (positiveSpan >= negativeSpan) {
                  targetDown =
                    config.down.defaultValue + smoothed * (positiveSpan || 1);
                } else {
                  targetDown =
                    config.down.defaultValue - smoothed * (negativeSpan || 1);
                }
            ...
            console.warn(
              "[Live2DCharacter] Failed to update mouth parameters:",
              error
            );
          }
        ...
        console.warn(
          "[Live2DCharacter] Fallback lip sync setup failed:",
          error
        );
      }
    };
    ...
    useEffect(() => {
      ...
      if (!modelPath) {
        console.log(
          "[Live2DCharacter] No model path provided, skipping initialization"
        );
        return;
      }
      ...
      const initLive2D = async () => {
        try {
          console.log(
            "[Live2DCharacter] Initializing with model path:",
            modelPath
          );
          ...
          if (appRef.current) {
            console.log(
              "[Live2DCharacter] Cleaning up existing PIXI application"
            );
            try {
              ...
              console.warn(
                "[Live2DCharacter] Error destroying PIXI app:",
                destroyError
              );
            }
            appRef.current = null;
          ...
          if (!app || !app.stage) {
            throw new Error(
              "PIXI Application or stage is not properly initialized"
            );
          }
          ...
          if (motionSyncEnabled) {
            try {
              const motionSyncUrl = modelPath.replace(
                ".model3.json",
                ".motionsync3.json"
              );
              console.log(
                "[Live2DCharacter] Attempting to load MotionSync from:",
                motionSyncUrl
              );
              ...
              if (response.ok) {
                ...
                if (
                  ...
                  console.log(
                    "[Live2DCharacter] MotionSync loaded successfully"
                  );
                } else {
                  console.warn(
                    "[Live2DCharacter] Model internal structure not ready for MotionSync"
                  );
                  motionSyncRef.current = null;
                }
              } else {
                console.log(
                  "[Live2DCharacter] MotionSync file not found, skipping MotionSync initialization"
                );
                motionSyncRef.current = null;
              }
            } catch (motionSyncError) {
              console.error(
                "[Live2DCharacter] MotionSync initialization failed:",
                motionSyncError
              );
              setMotionSyncEnabled(false);
              motionSyncRef.current = null;
            ...
            console.log(
              "[Live2DCharacter] MotionSync disabled due to previous errors"
            );
            motionSyncRef.current = null;
          }
      ...
      if (audioTrack && audioTrack.getMediaStreamTrack) {
        ...
        if (motionSync && motionSyncEnabled && !isDisconnectingRef.current) {
          try {
            ...
            Promise.resolve()
              .then(() => {
                if (!isDisconnectingRef.current && motionSync) {
                  ...
                  console.log(
                    "[Live2DCharacter] MotionSync audio playback started"
                  );
                  stopFallbackLipSync();
                }
              })
              .catch((playError) => {
                console.error(
                  "[Live2DCharacter] MotionSync play promise error:",
                  playError
                );
                setMotionSyncEnabled(false);
                stopFallbackLipSync();
            ...
            console.error(
              "[Live2DCharacter] MotionSync play error:",
              motionSyncPlayError
            );
            setMotionSyncEnabled(false);
            stopFallbackLipSync();
        ...
        audioTrack.getMediaStreamTrack().onended = () => {
          ...
          if (motionSync && !isDisconnectingRef.current) {
            try {
              ...
              console.error(
                "[Live2DCharacter] MotionSync reset error:",
                resetError
              );
            }
          }
    ...
    useEffect(() => {
      return () => {
        console.log(
          "[Live2DCharacter] Component unmounting, cleaning up all resources"
        );
          ...
          try {
            setTimeout(() => {
              try {
                ...
                console.error(
                  "[Live2DCharacter] MotionSync reset error during unmount:",
                  resetError
                );
              }
            }, 50);
        ...
        if (appRef.current) {
          try {
            ...
            console.error(
              "[Live2DCharacter] PIXI app destroy error:",
              destroyError
            );
          }
          appRef.current = null;
    ...
    if (!isClient) {
      return (
        <div
          className={cn(
            "relative h-full w-full bg-gradient-to-b from-blue-50 to-blue-100 dark:from-gray-900 dark:to-gray-800",
            className
          )}
        >
