...
class MainControlExtension(AsyncExtension):
    ...
    async def _on_asr_result(self, event: ASRResultEvent):
        ...
        diarization = event.metadata.get("diarization") if event.metadata else None
        if diarization and isinstance(diarization, dict):
            ...
            if segments:
                ...
                self.ten_env.log_info(
                    f"[MainControlExtension] diarization speaker={speaker} channel={channel} text={event.text}"
                )
        await self._send_transcript(
            "user",
            event.text,
