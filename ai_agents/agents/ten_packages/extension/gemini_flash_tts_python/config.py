...
class GeminiFlashTTSConfig(AsyncTTS2HttpConfig):
    ...
    def validate(self) -> None:
        ...
        if "api_key" not in self.params or not self.params["api_key"]:
            raise ValueError("API key is required for Gemini Flash TTS")
        if "model" not in self.params or not self.params["model"]:
            raise ValueError("Model is required for Gemini Flash TTS")
