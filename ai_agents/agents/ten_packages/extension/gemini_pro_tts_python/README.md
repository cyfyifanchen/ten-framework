# gemini_pro_tts_python

A Google Gemini 2.5 Pro TTS extension for the TEN framework, providing high-quality text-to-speech synthesis.

## Features

- Integration with Google Gemini 2.5 Pro TTS (optimized for quality)
...
- Ideal for podcasts, audiobooks, and high-fidelity content
...
| `model` | `string` | Model identifier (default: `gemini-2.5-pro-preview-tts`) |
| `voice` | `string` | Voice name (e.g., `Kore`, `Puck`, `Charon`) |
| `language_code` | `string` | Language code in BCP-47 format (e.g., `en-US`) |
| `style_prompt` | `string` | Style instructions for delivery (e.g., "professional and authoritative") |
...
## Differences from Gemini Flash TTS
...
| **Best For** | Voice assistants, chatbots | Professional content creation |
...
- [ ] Run `task lint-extension EXTENSION=gemini_pro_tts_python`
